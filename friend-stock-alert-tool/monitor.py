...
def github_actions_run_lines():
    ...
    lines = ["运行环境：GitHub Actions 云端"]
    if event:
        lines.append(f"触发方式：{event}")
    if repo and run_id:
        lines.append(f"运行链接：{server}/{repo}/actions/runs/{run_id}")
    return lines
...
def summarize_digest_articles(config, digest_config, articles):
    ...
    if should_use_openai_summary(config, digest_config):
        try:
            selected = select_and_summarize_news(config, articles, max_items=max_items)
            lines = format_summarized_news_items(selected, max_items=max_items)
            if lines:
                return lines
        except Exception as exc:
            print(f"OpenAI news summary failed: {safe_error_message(exc)}", file=sys.stderr, flush=True)

    selected = select_and_summarize_news_local(config, articles, max_items=max_items)
    return format_summarized_news_items(selected, max_items=max_items)


def should_use_openai_summary(config, digest_config):
    provider = digest_config.get("summary_provider")
    if provider is None:
        provider = config.get("translation", {}).get("summary_provider")
    if provider is None:
        provider = config.get("translation", {}).get("provider", "openai")
    return provider == "openai" and bool(openai_api_key(config))


def format_summarized_news_items(items, max_items=10):
    lines = []
...
def select_and_summarize_news_local(config, articles, max_items=10):
    ranked = sorted(
        enumerate(articles),
        key=lambda pair: (-score_digest_article(pair[1]), pair[0]),
    )
    selected = []
    seen_fingerprints = set()
    for _, article in ranked:
        fingerprint = news_fingerprint(article.get("title", ""))
        ...
        selected.append(
            {
                "title": title,
                "summary_zh": summarize_article_locally(article),
                "source": article.get("source") or "",
...
def news_fingerprint(title):
    ...
    for word in str(title).lower().replace("-", " ").split():
        ...
        if len(cleaned) >= 4 and cleaned not in {"stock", "stocks", "market", "markets", "says", "with"}:
            words.append(cleaned)
    return " ".join(words[:8])


def score_digest_article(article):
    title = str(article.get("title") or "").lower()
    keyword = str(article.get("keyword") or "").lower()
...
def summarize_article_locally(article):
    text = f"{article.get('title', '')} {article.get('keyword', '')}".lower()
    themes = []
    implications = []

    if any(term in text for term in ["hbm", "memory", "dram", "nand", "micron", "sk hynix", "samsung"]):
        themes.append("存储/HBM供应链")
        implications.append("可能影响HBM、DRAM/NAND供需和相关厂商预期")
    if any(term in text for term in ["nvidia", "gpu", "ai chip", "accelerator", "broadcom", "amd", "tsmc"]):
        themes.append("AI芯片")
        implications.append("可能影响AI芯片、代工或加速卡需求判断")
    if any(term in text for term in ["earnings", "guidance", "revenue", "profit", "margin"]):
        themes.append("业绩/指引")
        implications.append("可能影响盈利预期和估值重定价")
    if any(term in text for term in ["capex", "hyperscaler", "data center", "power", "infrastructure"]):
        themes.append("AI基础设施投入")
        implications.append("可能影响数据中心资本开支和上游供应链预期")
    if any(term in text for term in ["export control", "sanction", "tariff", "policy", "china", "korea"]):
        themes.append("政策/地缘风险")
        implications.append("可能影响芯片出口、供应链和区域市场风险偏好")
    if any(term in text for term in ["market", "kospi", "nasdaq", "sell-off", "pullback", "rally", "valuation"]):
        themes.append("市场情绪")
        implications.append("反映板块风险偏好或估值变化")

    if not themes:
        return "本地规则保留该新闻作为相关市场信息；未使用AI模型，具体事实请以原文为准。"

    theme_text = "、".join(dict.fromkeys(themes))
    implication_text = "；".join(dict.fromkeys(implications))
    return f"本地规则识别为{theme_text}相关新闻，{implication_text}。未使用AI模型，具体事实请以原文为准。"
...
def send_daily_digest(config, state, force=False):
    ...
    run_lines = github_actions_run_lines()
    body_parts = [
