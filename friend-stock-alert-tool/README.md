...
它会在 GitHub 云端每天按 `Asia/Shanghai` 时区 12:59 运行一次，所以你的电脑关机、合盖、断网都不影响。当前设置为 12:59 是为了临近时间测试 GitHub schedule 是否会触发；测试完成后可以再改回早盘时间。GitHub 官方说明定时任务在高负载时可能延迟，整点尤其常见。
